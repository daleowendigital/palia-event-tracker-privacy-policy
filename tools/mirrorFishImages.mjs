...
const PAGES_BASE_URL =
  ...
  "https://daleowendigital.github.io/palia-event-tracker-privacy-policy";

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

function pickArrayKey(raw) {
  if (Array.isArray(raw)) return { key: null, arr: raw };
  if (raw && Array.isArray(raw.fish)) return { key: "fish", arr: raw.fish };
  if (raw && Array.isArray(raw.items)) return { key: "items", arr: raw.items };
  if (raw && Array.isArray(raw.data)) return { key: "data", arr: raw.data };

  // fallback if it's an object keyed by id
  if (raw && typeof raw === "object") {
    const vals = Object.values(raw);
    if (vals.length && typeof vals[0] === "object") return { key: "__objectValues", arr: vals };
  }
  return { key: "fish", arr: [] };
}
...
async function fetchWithBackoff(url, opts = {}) {
  const { tries = 6, timeoutMs = 12000, maxTotalMs = 60000 } = opts;
  ...
  for (let i = 0; i < tries; i++) {
    ...
    try {
      ...
      console.log(
        `[mirror] fetch fail for ${url} (try ${i + 1}/${tries}) waiting ${wait}ms: ${String(e?.message || e)}`,
      );
      await sleep(wait);
    }
  }
...
async function main() {
  ...
  const { key, arr } = pickArrayKey(raw);
  const fish = arr;

  if (!fish.length) throw new Error("fish.json had no fish array");

  // Keep it polite.
  const CONCURRENCY = Number(process.env.MIRROR_CONCURRENCY || 1);
  const MIN_BYTES_OK = 1024;
  ...
  async function worker() {
    while (cursor < fish.length) {
      ...
      try {
        ...
        if (fs.existsSync(outPath) && fs.statSync(outPath).size > MIN_BYTES_OK) {
          skipped++;
        } else {
  ...
  const progressTimer = setInterval(() => {
    ...
    console.log(
      `[mirror] progress ${done}/${fish.length} (${pct}%) elapsed=${mins}m downloaded=${downloaded} skipped=${skipped} rewritten=${rewritten} missingSrc=${missingSrc} failed=${failed.length}`,
    );
  }, 5000);
  ...
  try {
    ...
    // Build output in SAME SHAPE as input where possible.
    let out;
    if (Array.isArray(raw)) {
      out = fish;
    } else if (key && key !== "__objectValues") {
      out = { ...raw, [key]: fish };
    } else {
      // If we couldn't confidently preserve shape, default to { fish: [...] }
      out = { fish };
    }
  ...
  console.log(`[mirror] downloaded=${downloaded} skipped=${skipped} rewritten=${rewritten} missingSrc=${missingSrc}`);
  if (missingSrc) {
    console.log(
      `[mirror] WARNING: ${missingSrc} entries had no imageUrl going in. If you expected wiki URLs first, run your enrich step before mirroring.`,
    );
  }
  if (failed.length) {
